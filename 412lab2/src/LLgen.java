...
public class LLgen {
	...
	public static void buildProductions(ArrayList<String> gramLine, int flagIndex){
		...
		if(!createdProduction.contains(productionString)){
			createdProduction.add(productionString);
			finalProduction.put(productionMapIndex, productionArrayListTop);
			productionMapIndex += 1;
		}
		
		if(!createdProduction.contains(productionString2)){
