...
class ThisMyDocumentDatabase {
	...
	private String[] stopWords = new String[] { "a", "about", "above", "above",
	...
	public void run(String uiHost, UUID uiKey) {
		...
		while (true) { // LeveL 000000000000000000000000
			...
			if (selection == 1 || selection == 2) {
				if (selection == 1) {
					System.out
							.println("Enter the directory where the corpus is located: ");
					...
					try {
						...
						System.out
								.println("An error has occurred with the file path...");
					}
					System.out.println("The home directory: "
							+ homeDirectoryFile.getName());
					System.out.println("The length of the file: "
							+ homeDirectoryFile.length());
					System.out
							.println("Counting the number of occurs of each word in the corpus...");
					// get all the words
					File[] fileString = new File[0]; // !!!!!!result.listFiles();
					// total number of files
					// int totalFiles = fileString.length;
					// loop through each file
					// !!!!!! for(int ithFile = 0; ithFile < totalFiles;
					// ithFile++){
					...
					System.out.println("Found " + totalUniqueWords
							+ " unique words in the corpus.");
					System.out
							.println("How many of those words do you want to use to index the docs");
					...
					ArrayList<String> fullOfWords = wordCount
							.getKthMostFrequent();
					...
					System.out
							.println("How many topics do you want to use to index the docs?");
					// used for the top k machine
					int numTopicsToIndex = input.nextInt();
					// input the number then shows up next
					System.out
							.println("What value should I use in the alpha vector?");
					double valAlphaVec = input.nextDouble();
					alphaVec = new SparseDoubleVector(numWordsToIndex,
							valAlphaVec);
					// input the alpha parameter
					System.out
							.println("What value should I use in the beta vector?");
					double valBetaVec = input.nextDouble();
					System.out.println("Loading Documents into LDALearner...");
					betaVec = new SparseDoubleVector(numTopicsToIndex,
							valBetaVec);
					myLearning = new SCLDALearner(alphaVec, betaVec, random);
					// ________________________________From the A8 Tester first
					// method________________________

					// this is the true word_prob matrix, and the initial one
					IDoubleMatrix trueWordProbs = new RowMajorDoubleMatrix(
							numTopicsToIndex, numWordsToIndex, 0.0);
					IDoubleMatrix initialWordProbs = new RowMajorDoubleMatrix(
							numTopicsToIndex, numWordsToIndex, 0.0);

					try {
						// and load it up with data... there is no overalap
						// among topics here
						for (int i = 0; i < numTopicsToIndex; i++) {
							IDoubleVector myWordProbs = new SparseDoubleVector(
									numWordsToIndex, 0.02);
							...
							Dirichlet myDirichlet = new Dirichlet(random,
									new DirichletParam(new SparseDoubleVector(
											numWordsToIndex, 1), 10e-99, 1000));
						...
						IDoubleVector wordsInDoc = new SparseDoubleVector(
								numWordsToIndex, 0.0);// >>
						...
						try {
							...
							while ((l = inputStream.readLine()) != null) {
								...
								if (l.contains("</doc>")) {
									//String s = some;
									Dirichlet myDirichlet = new Dirichlet(
											random,
											new DirichletParam(
													new SparseDoubleVector(
															numTopicsToIndex, 1),
													10e-99, 1000));
									IDoubleVector initialTopicProbs = myDirichlet
											.getNext();
									myLearning.loadUpAnotherDoc(some + "",
											wordsInDoc, initialTopicProbs);
									wordsInDoc = new SparseDoubleVector(
											numWordsToIndex, 0.0);
								} else {
									wordsInDoc = countWordInDocument(l,
											wordsInDoc);
								}
						...
						/********************Dirichlet myDirichlet = new Dirichlet(random,
								new DirichletParam(new SparseDoubleVector(
										numTopicsToIndex, 1), 10e-99, 1000));// >
						IDoubleVector initialTopicProbs = myDirichlet.getNext();// >

						myLearning.loadUpAnotherDoc(homeDirectoryFile + "",
								wordsInDoc, initialTopicProbs);// >**********************
*/						
					...
					// _______________________________________________From the
					// A8 Tester first method________________________
					...
					while (allDocuments.hasNext()) {
						...
						IDoubleVector wordVecDoc = singleDocument
								.getWordsInDoc();
					...
					System.out
							.println("And now I've initialized the word_probs matrix.");
					...
					while (true) {
						...
						while (true) {// second while loop
							System.out.println("Do you want to:");
							System.out
									.println("\t(1)Update/view/save the learned model");
							System.out
									.println("\t(2) Use the learned model to search the corpus");
							...
							if (selection == 1) {// "\t(1)Update/view/save the learned model"
								while (true) {
									System.out.println("Do you want to:");
									System.out
											.println("\t(1) Run some iterations of the learner");
									System.out
											.println("\t(2) Print out the current topics");
									System.out
											.println("\t(3) Write the current model to a file");
									System.out
											.println("\t(4) Stop updating/viewing/saving the model:");
									selection = input.nextInt();
									if (selection == 1) {// t(1) Run some
															// iterations of the
															// learner
										// selection 1 gets you this
										System.out
												.println("How many iters do you want to run?");
										// enter an integer and it gets you this
										// printed out based on the number of n
										// iterations
										...
										for (int i = 0; i < numIterations; i++) {
											...
											System.out.println("Running iter "
													+ i + " ..."); // THIS LOOPS
																	...
																	// INTERATIONS
											System.out
													.println("\tUpdating produced... ");// THIS
																						// LOOPS
																						// FROM
																						// 1
																						// TO
											...
											System.out
													.println("\tUpdating topic_probs... ");
											myLearning.updateTopicProbs();
											System.out
													.println("\tUpdating word_probs... ");
											myLearning.updateWordProbs();
										}
									}// ========================
									if (selection == 2) {// \t(2) Print out the
															// current topics
										// selection 2 gets you this from Print
										// out the current topics
										System.out
												.println("How many of the top words do you want for each topic?");
										// lets say that you input 5, you get 0
										// to n topics that five words long
										int numTopWordsEachTopic = input
												.nextInt();
										int ithTopic = 0;
										int numWordsInDict = wordDict.size();
										while (ithTopic < numTopicsToIndex) {
											AVLTopKMachine jmachine = new AVLTopKMachine(
													numTopWordsEachTopic);
											try {
												...
												for (int j = 0; j < numWordsInDict; j++) {
													...
													jmachine.insert(formula,
															wordDict.get(j));

												}
											} catch (OutOfBoundsException E) {
												System.out
														.println("Trouble inserting into the machine.");
											}

											ArrayList topicWords = jmachine
													.getTopK();
											...
											System.out.println("topic "
													+ ithTopic + topicString);
											ithTopic++;
										}

									}
									// THIS IS CORRECT
									if (selection == 3) {// \t(3) Write the
															// current model to
															// a file
										// selection 3 gets you this from (3)
										// Write the current model to a file"
										System.out
												.println("What diretory should I write to?");
										// select the file location in the
										// window pane, after that
										try {
											String homeDirectory = "/StratoDrive/";
											System.out.println(homeDirectory);
											System.out
													.println("What is the name of the XML file where the topic model should be written? (fileName.xml)");
											String thisFileXName = input.next();
											myLearning
													.writeToFile(homeDirectory
															+ thisFileXName);

											System.out
													.println("And where should the binary file with the other meta-data be written?");
									...
									if (selection == 4) {// \t(4) Stop
															// updating/viewing/saving
															// the model:"
										break;
									}
								}
							...
							if (selection == 2) {// (2) Use the learned model to
													// search the corpus
								System.out
										.println("OK. Building an M-Tree index...");
								SCMTree treeLearning = new SCMTree(3, 4);
								allDocuments = myLearning.iterator();
								while (allDocuments.hasNext()) {
									DocSignature singleDocument = allDocuments
											.next();
									treeLearning.insert(new MultiDimPoint(
											singleDocument.getTopicProbs()),
											singleDocument.getFName());
								}
								System.out
										.println("Done buidling an M-Tree index.");
								while (true) {// whileXXY

									System.out.println("Do you want to:");
									System.out
											.println("\t(1)Search the database");
									System.out
											.println("\t(2) Set the number of query results to return");
									System.out
											.println("\t(3)Stop searching the database:");
									selection = input.nextInt();
									if (selection == 1) {// (1)Search the
										System.out
												.println("Enter in the URL (name) to find related documents.");
										System.out.println("");
										String thisFileNameURL = input.next();
										WordMounter singleCounter = new WordMounter();

										readSingleLine(thisFileNameURL);
										Dirichlet myDirichlet = new Dirichlet(
												random,
												new DirichletParam(
														new SparseDoubleVector(
										...
										myLearning.loadUpAnotherDoc("query",
												wIDA, tPB);
										...
										while (countUp < 100) {
											...
											myLearning
													.updateTopicProbs("query");
											countUp++;
										}
										DocSignature getQueryDoc = myLearning
												.getSignature("query");
										ArrayList<DataWrapper<MultiDimPoint, String>> similarDocs = treeLearning
												.findKClosest(
														new MultiDimPoint(
																getQueryDoc
																		.getTopicProbs()),
														numResultsReturnFromQuery);
										int numDocsHere1 = similarDocs.size();
										ArrayList<String> docFeeder = new ArrayList<String>();

										for (int i = 0; i < numDocsHere1; i++) {
											docFeeder.add(similarDocs.get(i)
													.getData());
											// System.out.println("___________nope1______________");
										}

										for (String thisDocument : docFeeder) {
											System.out
													.println("=====================");
											System.out
													.println("=====================");
											...
											try {
												...
												while ((l = inputStream.readLine()) != null) {
													if (l.contains("<doc")) {
														...
														if(some.contains(thisDocument)){
															while ((l = inputStream.readLine()) != null && !l.contains("</doc>")){
											...
											//TODO
											System.out
													.println("-------Type 'k' and Press 'Enter' for next document----------");
											System.out.println("\n");
											String movingAlong = input.next();
										}
										System.out.println(homeDirectory);
									}
									if (selection == 2) {// (2)Set the number of
															// query results to
															// return
										System.out
												.println("How many results to return?");
										numResultsReturnFromQuery = input
												.nextInt();

									}
									if (selection == 3) {// t(3)Stop searching
															// the database:"
										selection = -1;// VERY IMPORTANT DO NOT
														// CHANGE, SINCE 3 AND 3
														// FOR SELECTION'S IT
														// WHILE STOP THE
														// PROGRAM IF 3 MAKE IT
														// TO OUTER LOOP
										break;// /breaks out whileXXY
									}
								}
							}// // End of this(2) Use the learned model to
								// search the corpus
				...
				// this is correct
				if (selection == 2) {// _____________________________________Second
										// Half_______________________________________________________________
					System.out.println("Where the corpus is located? ");
					...
					try {
						String homeDirectory = homeDirectoryFile.toString();// result.getCanonicalPath();
						System.out.println(homeDirectory);
						// }catch(IOException E){}
						...
						// secondOtherWordDicMap = (SaveComponents)
						// in.readObject();
						// saved = (SaveComponents) in.readObject();
						secondWordDic = (ArrayList<String>) in.readObject();
						secondOtherWordDicMap = (Map<String, Integer>) in
								.readObject();
						...
						IDoubleVector first = new SparseDoubleVector(
								secondNumberWords, secondAlpha);
						IDoubleVector second = new SparseDoubleVector(
								secondNumberTopics, secondBeta);
						secLearner = new SCLDALearner(first, second, random);
						System.out
								.println("______________the name of the xml: "
										+ secondLearnerString);
					...
					// secLearner = new SCLDALearner(secondAlpha, secondBeta,
					// random);
					// secLearner.loadFromFile(secondLearnerString);
					while (true) {
						while (true) {// second while loop
								...
								while (true) {
										...
										while (ithTopic < secondNumberTopics) {
											...
											try {
												...
												for (int j = 0; j < secondNumberWords; j++) {
													...
													double pt = secwordCount
															.giveMeVal(secondWordDic
																	.get(j))
															/ secSummationVal;
													double pc = secLearner
															.getWordProbs()
															.getEntry(ithTopic,
																	j);
													double formula = pt
									...
									if (selection == 3) {
										...
										try {
											...
											System.out.println(homeDirectory);
											System.out
													.println("What is the name of the XML file where the topic model should be written? (use fileName.xml)");
											String thisFileXName = input.next();
											secLearner
													.writeToFile(homeDirectory
															+ thisFileXName);
											...
											ObjectOutputStream out = new ObjectOutputStream(
													fileOut);
											SaveComponents saveForLater = new SaveComponents();
											// saveForLater.theWordDicArrayList
											// = secondWordDic;
											// saveForLater.theOtherWordDic =
											// secondOtherWordDicMap;
											// saveForLater.thenumberOfWords =
											// secondNumberWords;
											// saveForLater.thenumberOfTopics =
											// secondNumberTopics;
											// //saveForLater.theAlpha =
											// secondAlpha;
											// //saveForLater.theBeta =
											// secondBeta;
											// saveForLater.nameXMLFile =
											// homeDirectory+"\\"+"+thisFileName";
											// saveForLater.thesummation =
											// secSummationVal;
											// saveForLater.thewordCount =
											// wordCount;
							...
							if (selection == 2) {// (2) Use the learned model to
								...
								System.out
										.println("OK. Building an M-Tree index...");
								SCMTree treeLearning = new SCMTree(3, 4);
								Iterator<DocSignature> allDocuments = secLearner
										.iterator();
								while (allDocuments.hasNext()) {
									DocSignature singleDocument = allDocuments
											.next();
								...
								while (true) {
									...
									selection = input.nextInt();
									if (selection == 1) {// (1)Search the
										System.out.println("Where is the database?");
										try {
											homeDirectoryFile = fc.main(inputString);
										} catch (IOException e1) {
											System.out
										...
										System.out
												.println("Enter in the URL (name of file) to find related documents.");
										System.out.println("");
										String thisFileNameURL = input.next();
										WordMounter singleCounter = new WordMounter();

										readSingleLine2(thisFileNameURL);
										Dirichlet myDirichlet = new Dirichlet(
												random,
												new DirichletParam(
														new SparseDoubleVector(
										...
										secLearner.loadUpAnotherDoc("query",
												wIDA, tPB);
										...
										DocSignature getQueryDoc = secLearner
												.getSignature("query");
										System.out
												.println("How many results to return?");
							...
							}// End of this(2) Use the learned model to search
								// the corpus
							if (selection == 3) {// (3)Exit:
								System.exit(0);
							}// end of this (3)Exit:
	...
	public int countChris(String inMe, WordMounter counter) {
		Pattern pattern = Pattern.compile("[a-zA-Z_0-9]*");// [a-zA-Z_0-9] [\\S]
		...
		for (; matcher.find(); returnVal++) {
			try {
				...
				if (wordDictMap.containsKey(matcher.group().toLowerCase()) == true) {
					int numIndex = (int) wordDictMap.get(matcher.group()
							.toLowerCase());
		...
		try {
			...
			for (int j = 0; j < numWordsLine; j++) {
				...
				if (notWords.contains(wordsInLine[j]) != true) {
					...
					int incrementWord = wordDict.indexOf(wordsInLine[j]
							.toLowerCase());
					wIDA.setItem(incrementWord,
							wIDA.getItem(incrementWord) + 1.0);
				}
			}
			// }
		} catch (Exception e) {
		...
		try {
			...
			for (int j = 0; j < numWordsLine; j++) {
				...
				if (notWords.contains(wordsInLine[j]) != true) {
					...
					int incrementWord = secondWordDic.indexOf(wordsInLine[j]
							.toLowerCase());
					wIDA.setItem(incrementWord,
							wIDA.getItem(incrementWord) + 1.0);
				}
