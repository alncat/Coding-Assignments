...
/**
 * A JUnit test case class for testing an implementation of an IDoubleMatrix. Every method starting
 * with the word "test" will be called when running the test with JUnit.
 */
public class DoubleMatrixTester extends TestCase {
	...
	// An array of doubles of test values that we test every method on. These include the boundary
	// values from above, 0.0, average values, decimals, negative, maximum, minimum, positive and
	// negative inifinity.
	private final double[] testValues =
			{0.0, 1.0, -1.0, -100.0, 100.0, 15.1610912461267127317, -2372.3616123612361237132,
					MAX_DOUBLE, MIN_DOUBLE, POSITIVE_INFINITY, NEGATIVE_INFINITY};
	// An array of doubles of initial values that we test every method on. These include the
	// boundary values from above, 0.0, average values, and decimals.
	// Note that Positive and Negative infinity is excluded here. This is due to a problem with the
	// initialization of SparseDoubleVectors. If we initialize a
	// sparse double vector with infinity, changing it from infinity will result in NaN. Note that
	// NaN compared to NaN always results in false.
	private final double[] initValues = {0.0, 1.0, -1.0, -100.0, 100.0, 15.1610912461267127317,
			-2372.3616123612361237132, MAX_DOUBLE, MIN_DOUBLE};

	// Here, we have a different set of values for the "set values". We have to use these values as
	// opposed to the init values above due to precision and boundary issues.
	// For the boundary issues, when we utilize MAX_DOUBLE, but then set a value to a negative
	// number, the SpareDoubleVector will store a number that is less than the minimum possible
	// number
	// at this point. This will result in an overflow, and loop back around to 0. A similar thing
	// happens with MIN_DOUBLE and positive values.
	// On the other hand, when we test with numbers with many decimals, this leads to losses in
	// precision when we set values. Therefore, these small losses in precision will evaluate
	// to a miss when we compare it.
	private final double[] setValues = {0.0, -1.0, -6000000.0, 5000000000000.0};
	// An array of integers that represent indices we utilize when testing for failures (when we are
	// catching OutOfBoundaryExceptions)
	private final int[] failIndices = {-100, -1, 1000};
	// An array of integers that represent indices that work when we test for failures (when we are
	// catching OutOfBoundaryExceptions)
	private final int[] validIndices = {0, 99};
	// An array of integers that represent lengths of columns [rows] that would not be accepted by
	// an array of height [length] 100.
	private final int[] failLengths = {1, 99, 1000};

	/**
	 * This is used all over the place to check if an observed value is close enough to an expected
	 * double value
	 */
	private void checkCloseEnough(double observed, double goal) {
		if (!(observed > goal - 0.000001 && observed < goal + 0.000001)) {
	...
	public void testMatrixSize() {
		...
		for (int i = 0; i < 100; i += 10) {
			...
			assertEquals("Row major matrix size (rows) failed to match", i * 2,
					rMatrix.getNumRows());

			IDoubleMatrix cMatrix = new ColumnMajorDoubleMatrix(i * 2, i, 5.0);
			assertEquals("Column major matrix size (col) failed to match", i,
					cMatrix.getNumColumns());
			assertEquals("Column major matrix size (rows) failed to match", i * 2,
					cMatrix.getNumRows());
		}
	 ...
	 * the number of columns and number of rows. Precondition: numCols or numRows should be invalid
	 * (less than 0)
	 * 
	 ...
	 */
	private void checkAllMatrixElementsEqualTo(IDoubleMatrix matrix, int numCols, int numRows,
			double sameValue) {
	...
	/**
	 * Used to test a simple setting and getting of an element in a matrix. Checks for out-of-bounds
	 * access as well.
	 * 
	...
	/**
	 * Checks that an exception is thrown when an invalid index is accessed in the given matrix with
	 * setEntry(). Precondition: should be called with indices that are out of bounds for the given
	 * matrix, as defined in the specification.
	 * 
	 * @param matrix
	...
	/**
	 * Checks that an exception is thrown when an invalid index is accessed in the given matrix with
	 * getEntry(). Precondition: should be called with indices that are out of bounds for the given
	 * matrix, as defined in the specification.
	 * 
	 * @param matrix
	 * @param i column in matrix to be accessed
	...
	/**
	 * Tests that multiple elements can be rewritten in the matrix multiple times.
	 */
	...
	/**
	 * Initializes a matrix to a set of simple values. Note that this only initializes the top
	 * left-hand 4 columns and 3 rows; all other elements are left unchanged. Top elements of matrix
	 * will look like: counter counter+3 counter+6 counter+9
	 * 
	 ...
	 * Initializes a matrix to a set of simple values. Note that this only initializes the top
	 * left-hand 4 columns and 3 rows; all other elements are left unchanged. Top elements of matrix
	 * will look like: 1.0 4.0 7.0 10.0
	 * 
	 * 2.0 5.0 8.0 11.0
	 * 
	 * 3.0 6.0 9.0 12.0
	...
	/**
	 * Checks that all elements of a primitive 2-d array are equivalent to the elements found in a
	 * matrix. Note that if the matrix is larger than the primitive 2-d array, the elements in the
	 * matrix that do not have any corresponding elements in the 2-d array will not be checked
	 * 
	...
	/**
	 * Test if getRow() works as specified. Tests if OutOfBounds exceptions are thrown correctly
	 * with invalid access.
	 * 
	 * @param matrix
	...
	private void seeIfGetRowWorks(IDoubleMatrix matrix, double initialValue) {
		...
		try {
			...
			checkListsAreEqual(new double[] {3, 6, 9, 12, initialValue},
					matrix.getRow(matrix.getNumRows() - 1));
		} catch (OutOfBoundsException e) {
	...
	/**
	 * Checks that an exception is thrown when an invalid row is accessed in the given matrix with
	 * getRow(). Precondition: should be called with row index that is out of bounds for the given
	 * matrix, as defined in the specification.
	 * 
	 * @param matrix
	 * @param rowIndex
	...
	/**
	 * Tests if the getColumn() function works correctly, also checks for out of bounds access
	 * errors.
	 */
	...
	/**
	 * Test if getColumn() works as specified. Tests if OutOfBounds exceptions are thrown correctly
	 * with invalid access.
	 * 
	 * @param matrix
	 * @param initialValue the initial value of every element in the matrix
	 ...
	 * with getColumn(). Precondition: should be called with column index that is out of bounds for
	 * the given matrix, as defined in the specification.
	 * 
	 * @param matrix
	 * @param colIndex
	 */
	...
	/**
	 * Tests if the setColumn() function works correctly, also checks for out of bounds access or
	 * illegal argument errors which should be thrown under certain conditions.
	 */
	...
	/**
	 * Test if setColumn() works as specified. Tests if OutOfBounds exceptions are thrown correctly
	 * with invalid access; also tests if IllegalArgument exceptions are thrown when the
	 * IDoubleVector argument in setColumn() is null or not of the same length as the columns of the
	 * matrix
	 * 
	 * @param matrix
	 */
	private void seeIfSetColumnWorks(IDoubleMatrix matrix) {
		...
		// all of these cases should throw an exception - either invalid column indices are to be
		// set,
	...
	/**
	 * Tests if OutOfBounds exceptions are thrown correctly with invalid access; also tests if
	 * IllegalArgument exceptions are thrown when the IDoubleVector argument in setColumn() is null
	 * or not of the same length as the columns of the matrix.
	 * 
	 * Preconditions: IDoubleVector argument in setColumn() is null or not of the same length as the
	 * columns of the matrix or the column index exceeds the bounds of the matrix.
	 * 
	 * @param matrix
	 * @param columnIndex
	 * @param column
	 */
	private void throwsCorrectExceptionSetColumn(IDoubleMatrix matrix, int columnIndex,
			IDoubleVector column) {
		boolean errorNotThrown = true;

		// try an invalid setColumn at either an invalid index or with an invalid (null) argument,
	...
	/**
	 * Tests if the setRow() function works correctly, also checks for out of bounds access or
	 * illegal argument errors which should be thrown under certain conditions.
	 */
	...
	/**
	 * Test if setRow() works as specified. Tests if OutOfBounds exceptions are thrown correctly
	 * with invalid access; also tests if IllegalArgument exceptions are thrown when the
	 * IDoubleVector argument in setRow() is null or not of the same length as the columns of the
	 * matrix
	 * 
	 * @param matrix
	 */
	...
	private void throwsCorrectExceptionSetRow(IDoubleMatrix matrix, int rowIndex, IDoubleVector row) {
		...
		// try an invalid setRow at either an invalid index or with an invalid (null) argument,
		// which should throw an error.
		try {
	...
	/**
	 * Tests a more complex scenario of getting and setting columns and rows. Basically, see if
	 * overwriting an element in a column by setting a row works, and see if overwriting an element
	 * in a row by setting a column works properly.
	 * 
	...
	private void seeIfComplexSetAndGetWorks(IDoubleMatrix matrix) {
		...
		try {
			...
			checkListsAreEqual(new double[] {-999, -999, -999, 100, -999, -999},
					matrix.getColumn(3));
			...
			// check that getEntry returns the correct result at the intersection of this changed
			// row and column
	...
	/**
	 * Tests the matrix by writing few entries in a really large matrix. The dimensions of the
	 * matrix should preferably be atleast 500x500.
	 */
	...
	/**
	 * Tests the matrix by writing around 1% of the matrix in a sparse manner for a
	 * RowMajorDoubleMatrix
	 */
	...
	/**
	 * This test runs a test on the following combinations on a wide variety of matrix sizes and
	 * indices. Here, we test setting an entry, then setting a row. setting an entry, then setting a
	 * column. setting a row, then setting an entry. setting a column, then setting an entry.
	 */
	public void testEntryCombos() {
		int numRows;
		int numCols;
		// Change rows represents the row indices that we change to another value (distinct from the
		// initial value)
		int[] changeRows = {0, 0, 0};
		// Change columns represents the row indices that we change to another value (distinct from
		// the initial value)
		...
		for (int i = 1; i < 1000; i = i * 10) {
			for (int j = 0; j < 3; j++) {
				for (int k = 0; k < 3; k++) {
					...
					// Here, we test setting a row, then an entry (or the other way around) on all
					// possible combinations from
					// a corner to a boundary, a boundary to a corner, or even middle cases. We also
					// verify that when they don't
							...
							for (int changeRow : changeRows) {
								...
								for (int change_i_Entry : changeCols) {
									for (int change_j_Entry : changeRows) {
										// Test setting an entry and then the row on a column major
										// double matrix
										myMatrix =
												new ColumnMajorDoubleMatrix(numRows, numCols, 0.0);
										setRow = new SparseDoubleVector(numCols, testVal);
										testSetEntryRowCombo(myMatrix, change_i_Entry,
												change_j_Entry, changeRow, changeVal, setRow);

										// Test setting a row and then the entry on a column major
										// double matrix
										myMatrix =
												new ColumnMajorDoubleMatrix(numRows, numCols, 0.0);
										setRow = new SparseDoubleVector(numCols, testVal);
										testSetRowEntryCombo(myMatrix, change_i_Entry,
												change_j_Entry, changeRow, changeVal, setRow);

										// Test setting an entry and then the row on a row major
										// double matrix
										...
										// Test setting a row and then the entry on a row major
										// double matrix
										myMatrix = new RowMajorDoubleMatrix(numRows, numCols, 0.0);
										setRow = new SparseDoubleVector(numCols, testVal);
										testSetRowEntryCombo(myMatrix, change_i_Entry,
					...
					// We use values from setValues due to the reason we mention at the top.
					// Here, we test setting a column, then an entry (or the other way around) on
					...
					for (double testVal : setValues) {
						for (double changeVal : setValues) {
							...
							for (int changeCol : changeCols) {
								...
								for (int change_i_Entry : changeCols) {
									for (int change_j_Entry : changeRows) {
										// Test setting an entry and then the column on a column
										// major double matrix
										myMatrix =
												new ColumnMajorDoubleMatrix(numRows, numCols, 0.0);
										setCol = new SparseDoubleVector(numRows, testVal);
										testSetEntryColCombo(myMatrix, change_i_Entry,
												change_j_Entry, changeCol, changeVal, setCol);

										// Test setting a column and then the entry on a column
										// major double matrix
										myMatrix =
												new ColumnMajorDoubleMatrix(numRows, numCols, 0.0);
										setCol = new SparseDoubleVector(numRows, testVal);
										testSetColEntryCombo(myMatrix, change_i_Entry,
												change_j_Entry, changeCol, changeVal, setCol);

										// Test setting an entry and then the column on a row major
										// double matrix
										myMatrix = new RowMajorDoubleMatrix(numRows, numCols, 0.0);
										setCol = new SparseDoubleVector(numRows, testVal);
										testSetEntryColCombo(myMatrix, change_i_Entry,
										...
										// Test setting a column and then the entry on a row major
										// double matrix
										myMatrix = new RowMajorDoubleMatrix(numRows, numCols, 0.0);
										setCol = new SparseDoubleVector(numRows, testVal);
										testSetColEntryCombo(myMatrix, change_i_Entry,
	...
	/**
	 * This test runs a test on the following combinations on a wide variety of matrix sizes and
	 * indices. Here, we test setting a column, then setting a row. setting a row, then setting a
	 * column.
	 */
		...
		for (int i = 1; i < 1000; i = i * 1000) {
			for (int j = 0; j < 3; j = j + 2) {
				for (int k = 0; k < 3; k = k + 2) {
					...
					// We use values from setValues due to the reason we mention at the top.
					// Here, we test setting a column, then a row (or the other way around) on all
					// possible combinations from
					// a boundary to a boundary, or a boundary to a middle, to even middle to
					...
					for (double testVal : setValues) {
						for (double changeVal : setValues) {
							...
							for (int changeRow : changeRows) {
								// Iterate through middle, leftmost, and rightmost columns
								for (int changeCol : changeCols) {
									// Test setting a row and then a column on a column major double
									// matrix
									...
									// Test setting a column and then a row on a column major double
									// matrix
									myMatrix = new ColumnMajorDoubleMatrix(numRows, numCols, 0.0);
									setRow = new SparseDoubleVector(numCols, testVal);
									setCol = new SparseDoubleVector(numRows, changeVal);
									testColRowCombo(myMatrix, changeCol, changeRow, setRow, setCol);
									// Test setting a row and then the column on a row major double
									// matrix
									myMatrix = new RowMajorDoubleMatrix(numRows, numCols, 0.0);
									setRow = new SparseDoubleVector(numCols, testVal);
									setCol = new SparseDoubleVector(numRows, changeVal);
									testRowColCombo(myMatrix, changeCol, changeRow, setRow, setCol);
									// Test setting a column and then a row on a row major double
									// matrix
									myMatrix = new RowMajorDoubleMatrix(numRows, numCols, 0.0);
									setRow = new SparseDoubleVector(numCols, testVal);
									setCol = new SparseDoubleVector(numRows, changeVal);
	...
	/**
	 * Here, we test first setting an entry in the matrix, then setting a row within the matrix. We
	 * check that the row is correct as well as the entry we changed within the matrix.
	 * 
	 ...
	 */
	private void testSetEntryRowCombo(IDoubleMatrix myMatrix, int i_entry, int j_entry, int j_row,
			double entry_val, IDoubleVector row_val) {
		...
		// We wrap it in a try statement since if we catch an OutOfBoundsException, then something
		// went wrong.
		try {
			// Set the correct value to be initially the entry value.
			correct_val = entry_val;
			// Note though that since we change the row later, if the row that we change contains
			// the index of the
			// entry value that we will set, then the correct value will be changed to what is
	...
	/**
	 * Here, we test first setting a row in the matrix, then setting an entry within the matrix. We
	 * check that the row is correct as well as the entry we changed within the matrix.
	 * 
	 * @param myMatrix The matrix that is passed in
	 * @param i_entry The column for which we set the entry
	 ...
	 * @param row_val the row that we will setRow with
	 */
	private void testSetRowEntryCombo(IDoubleMatrix myMatrix, int i_entry, int j_entry, int j_row,
			double entry_val, IDoubleVector row_val) {
		// An IDoubleVector that contains the correct row
		IDoubleVector correct_row;
		// A double that keeps track of what the correct value is.
	 ...
	 */
	private void testSetEntryColCombo(IDoubleMatrix myMatrix, int i_entry, int j_entry, int i_col,
			double entry_val, IDoubleVector col_val) {
		...
		try {
			...
			// Note though that since we change the column later, if the column that we change
	...
	/**
	 * Tests first setting a column in the matrix, then setting an entry within the matrix. We check
	 * that the column is correct as well as the entry we changed within the matrix.
	 * 
	 * @param myMatrix The matrix that is passed in
	 * @param i_entry The column for which we set the entry
	 * @param j_entry The row for which we set the entry
	 ...
	 * @param col_val The column that we will setColumn with
	 */
	private void testSetColEntryCombo(IDoubleMatrix myMatrix, int i_entry, int j_entry, int i_col,
			double entry_val, IDoubleVector col_val) {
		IDoubleVector correct_col;
		// A double that keeps track of what the correct value is.
		double correct_val;
	...
	/**
	 * Tests first setting a column in the matrix, then setting a row within the matrix. We check
	 * that the column is correct as well as the row we changed within the matrix.
	 * 
	 * @param myMatrix The matrix that is passed in
	 * @param i_col The index of the row that we will set
	 ...
	 * @param col_val the column that we will setColumn with
	 */
	private void testRowColCombo(IDoubleMatrix myMatrix, int i_col, int j_row,
			IDoubleVector row_val, IDoubleVector col_val) {
		// IDoubleVectors that hold the correct column and the correct row
		IDoubleVector correct_col;
		IDoubleVector correct_row;

		try {
			// The correct row is set to the passed in column and won't change since we change the
			// row after we change the column.
			correct_row = row_val;
			// The correct column is set to the passed in column (will change since row and column
			// will always overlap)
			correct_col = col_val;

			// Now, we set the column and the row to the column/row passed in to the desired
			// indices.
			myMatrix.setColumn(i_col, col_val);
			myMatrix.setRow(j_row, row_val);
			...
			// We verify here that the column and row match what should be the correct column and
			// row.
			testGetColumn(myMatrix, i_col, correct_col);
			testGetRow(myMatrix, j_row, correct_row);
		} catch (OutOfBoundsException e) {
	 ...
	 * @param col_val the column that we will setColumn with
	 */
	private void testColRowCombo(IDoubleMatrix myMatrix, int i_col, int j_row,
			IDoubleVector row_val, IDoubleVector col_val) {
		// IDoubleVectors that hold the correct column and the correct row
		IDoubleVector correct_col;
		IDoubleVector correct_row;
		...
		try {
			// The correct row is set to the passed in column initially (will change since row and
			// column will always overlap)
			correct_row = row_val;
			// The correct column is set to the passed in column and won't change since we change
			// the column after we change the row.
			correct_col = col_val;
	...
	/**
	 * Tests that getRow works. We do so by comparing the row retrieved by getRow with an
	 * IDoubleVector that represents the row we expect to get.
	 * 
	...
	private void testGetRow(IDoubleMatrix myMatrix, int j, IDoubleVector expectedRow) {
		...
		try {
			...
			// In order to test that the retrieved row is correct, we iterate through every element
			// in the retrieved row and compare it
			// with the expected row
			for (int i = 0; i < myMatrix.getNumColumns(); i++) {
				assertTrue("getRow() Failed: Retrieved row did not match expected row",
						retrievedRow.getItem(i) == expectedRow.getItem(i));
			}
		} catch (OutOfBoundsException e) {
	...
	/**
	 * Tests that getEntry works. We do so by utilizing getEntry, then verifying that the entry
	 * retrieved matches what we expected.
	 * 
	 * @param myMatrix Matrix that is tested
	...
	private void testGetEntry(IDoubleMatrix myMatrix, int i, int j, double expectedVal) {
		// Attempt to retrieve entry. If it does not match, we print an appropriate error. If we
		// encounter an OutOfBoundsException, we print a different error.
		try {
			assertTrue("getEntry() Failed: Retrieved value did not match expected value",
					myMatrix.getEntry(j, i) == expectedVal);
		} catch (OutOfBoundsException e) {
	...
	/**
	 * Tests that getColumn works. We do so by comparing the column retrieved by getColumn with an
	 * IDoubleVector that represents the column we expect to get.
	 * 
	 * @param myMatrix Matrix that is tested
	...
	private void testGetColumn(IDoubleMatrix myMatrix, int i, IDoubleVector expectedCol) {
		...
		try {
			...
			// In order to test that the retrieved column is correct, we iterate through every
			// element in the retrieved column and compare it
			// with the expected column
			for (int j = 0; j < myMatrix.getNumRows(); j++) {
				assertTrue("getColumn() Failed: Retrieved column did not match expected column",
						retrievedColumn.getItem(j) == expectedCol.getItem(j));
			}
		} catch (OutOfBoundsException e) {
			// We fail instantly if we ever encounter an OutOfBoundsException
	...
	private void makeSureAdditionResultIsCorrect(IDoubleMatrix first, IDoubleMatrix second,
		...
		// go through and make sure that second has the sum, and that first and firstArray are the
		// same
		try {
	...
	private void makeSureSumColumnsIsCorrect(IDoubleVector res, int numRows, int numColumns,
			double[][] firstArray) {
	...
	private void makeSureSumRowsIsCorrect(IDoubleVector res, int numRows, int numColumns,
			double[][] firstArray) {
