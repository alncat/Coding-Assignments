...
/**
 * Update strategy that causes balls to interact with each other upon collision
 * @author jjc7, jfe2
 *
 */
public class CCStrategy extends  AUpdateStrategy {



@Override
/**
 * Updates the state of the context ball by calculating reduced mass,
 * impulse, and updating the collision
 * 
 *  @param context
 *  	The context ball
 *  @param dispatcher
...
public void updateState(final Ball context, Dispatcher dispatcher) {
			context.setVelocity(new Point(0, 0)); /*Makes the ball stationary.*/
		...
		public void apply(Ball other, Dispatcher disp) {
			if (context != other){
				if(context.getLocation().distance(other.getLocation())<=context.getRadius()+other.getRadius()){
					...
					Point2D.Double imp = impulse(context.getLocation(), context.getVelocity(), 
							other.getLocation(), other.getVelocity(), rMass, 
...
protected void updateCollision(Ball context, Ball target, double impX,
		double impY, Dispatcher dispatcher) {
