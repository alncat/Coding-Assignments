...
public class ClientGUI extends JFrame {
	...
	private JButton randomNumBtn;
	private JButton isEvenBtn;
	private JButton btnNewButton_2;
	private JTextField randomNumTF;
	private JLabel lblRange;
	private JTextField checkEvenTF;
	...
	protected void initGUI() {
		...
		panel = new JPanel();
		scroll.setColumnHeaderView(panel);
		
		randomNumBtn = new JButton("Generate Random Number");
		randomNumBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					append("Generate Random Number: ");
					append(model.generateRandomNum(Integer.parseInt(randomNumTF.getText()))+"\n");
				}
				catch(Exception excpt) {
					append("Generate Random Number exception: "+excpt+"\n");
				}
			}
		});
		...
		panel.add(randomNumTF);
		randomNumTF.setColumns(10);
		panel.add(randomNumBtn);
		
		isEvenBtn = new JButton("Is Even?");
		isEvenBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					append("Checking if the number is even: ");
					append(model.checkEven(Integer.parseInt(checkEvenTF.getText()))+"\n");
				}
				catch(Exception excpt) {
					append("Even Number Checking exception: "+excpt+"\n");
				}
			}
		});
		...
		checkEvenTF = new JTextField();
		panel.add(checkEvenTF);
		checkEvenTF.setColumns(10);
		panel.add(isEvenBtn);
		
		btnNewButton_2 = new JButton("Is Perfect Square?");
		btnNewButton_2.setToolTipText("Checks to see if a real number has a perfect square root.");
