...
import client.model.task.GetInfo;
import client.model.task.Pi2;
import client.model.task.RandomNum;
import client.model.task.isEven;
import client.model.task.isPerfectSquare;
...
public class ClientModel {
	 ...
	 * @param range
	 * @return A status string on generation
	 */
	public String generateRandomNum(int range){
		String result = "";
		if(null == comp)
			return "No ICompute object yet!";
		RandomNum task = new RandomNum(range);
		
		try{
			Integer randomNum = comp.executeTask(task);
			result = Integer.toString(randomNum);
		} catch (Exception e){
			view.append("\nGenerateRandomNum exception: " + e + "\n");
			e.printStackTrace();
		}
		return result;
	}
	...
	public String checkEven(int num){
		...
		if(null == comp)
			return "No ICompute object yet!";
		isEven task = new isEven(num);
		
		try{
			Boolean checkEven = comp.executeTask(task);
			result = checkEven.toString();
		} catch (Exception e){
			view.append("\nGenerate isEven exception: " + e + "\n");
			e.printStackTrace();
		}
		return result;
	}
	...
	public String isPerfect(int num){
		...
		if(null == comp)
			return "No ICompute object yet!";
		isPerfectSquare task = new isPerfectSquare(num);
		
		try{
			Boolean checkPerfectSquare = comp.executeTask(task);
			result = checkPerfectSquare.toString();
		} catch (Exception e){
			view.append("\nGenerate isPerfectSquare exception: " + e + "\n");
			e.printStackTrace();
		}
		return result;
	}
