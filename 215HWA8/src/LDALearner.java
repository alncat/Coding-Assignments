...
public class LDALearner implements ILDALearner {
	...
	// for the method updateProduced(), remembers the document position that was
	// retrived
	...
	public LDALearner(IDoubleVector alphaIN, IDoubleVector betaIN,
			IPRNG objectIN) {

		alpha = alphaIN;// the word dictionary that is passed in and has a
	...
	// Using the specified file, this loads the words_in_doc and topic_probs
	// vectors for
	// every document, as well as the word_probs vector for every topic. All of
	// the data
	// in the LDA object before loadFromFile is called should be deleted. Note
	// that after
	// calling "loadFromFile", the produced matrix for each document does not
	// exist
	...
	// this loads a document into the machine learning algorithm... "topicProbs"
	// is the initial
	// version of the topic_probs vector for that document, "wordsInDoc" is the
	// words_in_doc
	// vector for that document. Often, topicProbs will be nothing more than a
	// sample from
	// a Dirichlet distribution where the parameter is a vector of ones; this
	// provides a random
	// initialization
	public void loadUpAnotherDoc(String docName, IDoubleVector wordsInDoc,
			IDoubleVector topicProbs) {

		// Set up a new document with topic probability and words in the
		// document
		newDocSig = new DocSignature(docName, topicProbs, wordsInDoc);
		// Add the DocSignature to the data wrapper so that it can be added all
		// the other documents
	...
	// this loads all of the current topics into the machine learning
	// algorithm... the t^th row
	// of the matrix "loadMe" is the word_probs vector associated with the t^th
	// topic. Often,
	// wordProbs will initilly be nothing more than a random guess at the true
	// wordProbs matrix,
	// with each row sampled from a Dirichlet () distribution where the
	// parameter is a vector of
	...
	/**
	 * THESE METHODS ARE USED TO UPDATE THE DATA AS REQUIRED BY THE LDA
	 * ALGORITHM... SEE SLIDES 46 THROUGH 53 OF THE LECTURE 28 SLIDES
	 */

	// update/create the produced matrix for a particular document... if the doc
	// does not exist
	// or the word_probs matrix does not exist (it has not been loaded via
	// "loadUpWordProbs" or
	...
	public void updateProduced(String docName) {
		...
		try {
			...
			IDoubleMatrix newproduced = new ColumnMajorDoubleMatrix(
					numberOfRows, numberOfCols, 0.0);// yes
			// loop through all the columns in the produced matrix
			for (int col = 0; col < numberOfCols; col++) {
				// create a vector probs with the length of the topic probs
				// vectors in the DocSignatures
				...
				for (int word = 0; word < numberOfRows; word++) {
					double a = computedWordProbs.getEntry(word, col);// wordProbsGuess.getEntry(word,
																		// col);//needs
																		// to
																		// change
				...
				IRandomGenerationAlgorithm<IDoubleVector> multi = new Multinomial(
						prng, new MultinomialParam((int) docSign2
								.getWordsInDoc().getItem(col), probs));
				// Set the column for the matrix here
				newproduced.setColumn(col, multi.getNext());// yes
			}
			// add the final product of the newproduced matrix into the list
			// with the other matrices
			String rememberDocName = docName; // documentHolder.get(rememberDocPosition).getDocName();
			// create a new wrapper that has the updated produce or topic probs
			// matrix
			HoldDoc docSigTopicProbChange = new HoldDoc(docName, docSign2,
					newproduced);
			...
			System.out
					.println("_____________Error in method updatedProduced with string input____________");
		}
	}

	// update/create the produced matrix for ALL of the documents... just as
	// above, if the
	...
	// this updates the topic_probs vector for a particular document... if the
	// doc does not exist,
	// or the produced matrix for that doc does not exist (because
	// "updateProduced" has not been
	// called on this document since it was added or loaded) then an exception
	// is thrown
	public void updateTopicProbs(String docName) {
		try {
			...
			IRandomGenerationAlgorithm<IDoubleVector> dirich = new Dirichlet(
					prng, new DirichletParam(counter, 10e-40, 150));
			// grap the words in doc
			IDoubleVector holdWordsInDoc = docHolded2.getDocSig()
					.getWordsInDoc();
			// grap the produced matrix (Topic Probs Matrix)
			IDoubleMatrix holdProducedMatrixHere = docHolded2
					.getTopicProbsMatrix(); // documentHolder.get(docPosition).getTopicProbsMatrix();
			// setup a new document with the new topic probability
			DocSignature newDocSig = new DocSignature(docName,
					dirich.getNext(), holdWordsInDoc);
			// create new wrapper
			HoldDoc newWrap = new HoldDoc(docName, newDocSig,
					holdProducedMatrixHere);
			// remove the old value if needed?
			treeDocs.remove(docName);
			// update the topic probs matrix for the docsignature in tree
			...
			System.out
					.println("_______________eerrr updateTopicProbs(one parameter) gone array!!!_______________");
		}
	}

	// this updates the topic_probs vector for ALL of the documents... just as
	// above, if the
	// produced matrix does not exist for ANY of the documents in the corpus,
	// then an exception
	// is thrown
	...
	// this updates the word_probs vector for all of the topics... if the
	// produced matrix does
	...
	public void updateWordProbs() {
		try {
			...
			computedWordProbs = new RowMajorDoubleMatrix(numTopicsHere,
					checkDocHere.getTopicProbsMatrix().getNumColumns(), 0.0);
			// go through all the topics in the documents
			for (int topic = 0; topic < numTopicsHere; topic++) {
				// create a vector called counter
				IDoubleVector counter = new SparseDoubleVector(
						computedWordProbs.getNumColumns(), 0.0);
				// all the docsignature in the tree
				...
				while (hoo.hasNext()) {
					...
					IDoubleVector vectorTopic = docVectorHold
							.getTopicProbsMatrix().getRow(topic);
				...
				IRandomGenerationAlgorithm<IDoubleVector> dirich = new Dirichlet(
						prng, new DirichletParam(counter, 10e-40, 1500));
				// computed word probability that is not a guess
				computedWordProbs.setRow(topic, dirich.getNext());
			}
		} catch (OutOfBoundsException E) {
			System.out
					.println("______________________The words no update in method updateWordProbs______________");
		}
	}
	...
	// gets the current topic_probs vector and the words_in_doc vector for a
	// particular document...
	...
	public Iterator<DocSignature> iterator() {
		...
		// loop through the arrayList that has the HoldDocs and extract the
		// DocSignatures
	...
	// gets all of the word_probs vectors from the ML algorithm... the t^th row
	// in the matrix
	// corresponds to the word_probs vector for the t^th topic
	public IDoubleMatrix getWordProbs() {// this may have to come from the XML
											// file
		return computedWordProbs;
	}

	// this writes the data maintained by the ML algorithm to a file
	// (specifically, it writes all
	// of the topic_probs vectors, all of the words_in_doc vetors, and all of
	// the word_probs vectors).
	// If one calls "writeToFile", then creates a new LDA object and calls
	// "readFromFile" using that
	// same file, the new LDA object will have exactly the same contents as the
	// old one, except that
		...
		try {
			...
			while (ithTopic < numberOfRows) {
				...
				while (ithWord1 < numberOfCols) {
					HoldDoc anotherDoc = (HoldDoc) treeDocs.get(Integer
							.toString(ithWord1));
					DocSignature someDocSign1 = anotherDoc.getDocSig();
					IDoubleVector someTopicVec1 = someDocSign1.getTopicProbs();
					try {
						writer.println("<entry><dim>" + ithWord1
								+ "</dim><val>"
								+ someTopicVec1.getItem(ithWord1)
								+ "</val></entry>");
					} catch (OutOfBoundsException E) {
			...
			while (itDoc1 < countDocs) {
				...
				writer.println("<non_zero_cnt>" + numberOfRows
						+ "</non_zero_cnt>");
				HoldDoc anotherDoc = (HoldDoc) treeDocs.get(Integer
						.toString(itDoc1));
				DocSignature someDocSign1 = anotherDoc.getDocSig();
				...
				for (int i = 0; i < someWordVec1.getLength(); i++) {
					try {
						writer.println("<entry><dim>" + i + "</dim><val>"
								+ (int) someWordVec1.getItem(i)
								+ "</val></entry>");// line 36??
					} catch (OutOfBoundsException E) {
				...
				writer.println("<length>" + finalVeco22.getLength()
						+ "</length>");
				for (int ij = 0; ij < finalVeco22.getLength(); ij++) {
					try {
						writer.println("<entry><dim>" + ij + "</dim><val>"
								+ finalVeco22.getItem(ij) + "</val></entry>"); // some
																				// loop
																				// here
					} catch (OutOfBoundsException E) {
