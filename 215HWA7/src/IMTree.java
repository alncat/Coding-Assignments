...
interface IMTree<Key extends IPointInMetricSpace<Key>, MyData> {

	// insert a new key/data pair into the map
	void insert(Key keyToAdd, MyData dataToAdd);
	...
	ArrayList<DataWrapper<Key, MyData>> find(Key query, double distance);
	...
	// (NOT a null!)
	ArrayList<DataWrapper<Key, MyData>> findKClosest(Key query, int k);
