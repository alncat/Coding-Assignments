...
public class MTreeTester extends TestCase {
	...
	private void testOneDimRangeQuery(boolean orderThemOrNot, int minDepth, int internalNodeSize,
			int leafNodeSize, int numPoints, double expectedQuerySize) {

		// create two trees
		Random rn = new Random(133);
		IMTree<OneDimPoint, String> myTree =
				new SCMTree<OneDimPoint, String>(internalNodeSize, leafNodeSize);
		IMTree<OneDimPoint, String> hisTree =
				new MTree<OneDimPoint, String>(internalNodeSize, leafNodeSize);

		// add a bunch of points
		if (orderThemOrNot) {
			for (int i = 0; i < numPoints; i++) {
				double nextOne = i / (numPoints * 1.0);
				myTree.insert(new OneDimPoint(nextOne * numPoints),
						(new Double(nextOne * numPoints)).toString());
				hisTree.insert(new OneDimPoint(nextOne * numPoints), (new Double(nextOne
						* numPoints)).toString());
			}
		} else {
			for (int i = 0; i < numPoints; i++) {
		...
		assertTrue("Why was the tree not of depth at least " + minDepth,
				hisTree.depth() >= minDepth);

		// run a range query
		OneDimPoint query = new OneDimPoint(numPoints * 0.5);
		ArrayList<DataWrapper<OneDimPoint, String>> result1 =
				myTree.find(query, expectedQuerySize / 2);
		ArrayList<DataWrapper<OneDimPoint, String>> result2 =
				hisTree.find(query, expectedQuerySize / 2);
	...
	private void testOneDimTopKQuery(boolean orderThemOrNot, int minDepth, int internalNodeSize,
			int leafNodeSize, int numPoints, int querySize) {
		...
		if (orderThemOrNot) {
			...
			for (int i = 0; i < numPoints; i++) {// IN THE DATA, ONLY DOUBLE ARE BEING TURNED INTO
													// STRINGS TO ACT AS DATA
				double nextOne = rn.nextDouble();
				myTree.insert(new OneDimPoint(nextOne * numPoints),
						(new Double(nextOne * numPoints)).toString());
				hisTree.insert(new OneDimPoint(nextOne * numPoints), (new Double(nextOne
		...
		ArrayList<DataWrapper<OneDimPoint, String>> result2 =
				hisTree.findKClosest(query, querySize);
	...
	private void testMultiDimRangeQuery(boolean orderThemOrNot, int minDepth, int numDims,
		...
		// create two trees
		Random rn = new Random(133);
		IMTree<MultiDimPoint, String> myTree =
				new SCMTree<MultiDimPoint, String>(internalNodeSize, leafNodeSize);
		IMTree<MultiDimPoint, String> hisTree =
				new MTree<MultiDimPoint, String>(internalNodeSize, leafNodeSize);
		...
		if (orderThemOrNot) {
			...
			// do a top k query
			ArrayList<DataWrapper<MultiDimPoint, String>> result1 =
					myTree.findKClosest(query1, (int) expectedQuerySize);
			ArrayList<DataWrapper<MultiDimPoint, String>> result2 =
					myTree.findKClosest(query2, (int) expectedQuerySize);
	...
	private void testMultiDimTopKQuery(boolean orderThemOrNot, int minDepth, int numDims,
		...
		// run a top k query
		ArrayList<DataWrapper<MultiDimPoint, String>> result1 =
				myTree.findKClosest(query1, querySize);
		ArrayList<DataWrapper<MultiDimPoint, String>> result2 =
				hisTree.findKClosest(query1, querySize);
	...
	/**
	 * "TIER 1" TESTS COLLECTIVELY, THESE ARE WORTH 40% OF THE OVERALL "TEST CASE POINTS" NONE OF
	 * THESE REQUIRE ANY SPLITS
	 */
	...
	/**
	 * "TIER 2" TESTS COLLECTIVELY, THESE ARE WORTH 20% OF THE OVERALL "TEST CASE POINTS" NONE OF
	...
	/**
	 * "TIER 3" TESTS COLLECTIVELY, THESE ARE WORTH 20% OF THE OVERALL "TEST CASE POINTS" THESE
	 * REQUIRE A SPLIT OF AN INTERNAL NODE
	 */
	...
	// //_______________________________Code will not pass these tests
	...
	// ___________________________________Code will not pass these tests
