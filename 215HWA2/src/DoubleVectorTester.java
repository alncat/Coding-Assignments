...
public class DoubleVectorTester extends TestCase {
  ...
  /**
   * In this part of the code we have a number of helper functions that will be used by the actual
   * test functions to test specific functionality.
   */
  ...
  private void checkCloseEnough(double observed, double goal, int pos) {
    if (!(observed >= goal - 1e-8 - Math.abs(goal * 1e-8) && observed <= goal + 1e-8
        + Math.abs(goal * 1e-8)))
      if (pos != -1)
  ...
  /**
   * This adds a bunch of data into testMe, then checks (and returns) the l1norm
   */
  ...
  /**
   * This does a large number of setItems to an array of double vectors, and then makes sure that
   * all of the set values are still there
   */
    ...
    for (int i = 0; i < allMyVectors[0].getLength(); i++) {
      ...
      for (int j = 0; j < numVecs; j++) {
        try {
          ...
          fail("Got an out of bounds exception on set/get pos " + whichOne
              + " not expecting one!\n");
        }
      }
    }
  ...
  /**
   * This sets only the first value in a double vector, and then checks to see if only the first
   * vlaue has an updated value.
   */
  ...
  /**
   * This uses the l1NormTester to set a bunch of stuff in the input DoubleVector. It then
   * normalizes it, and checks to see that things have been normalized correctly.
   */
  ...
  /**
   * Here we have the various test functions, organized in increasing order of difficulty. The code
   * for most of them is quite short, and self-explanatory.
   */
   ...
   * pseudo-randomly. Then it adds each of them, in turn, into a single dense double vector, which
   * is then tested to see if everything adds up.
   */
